...
class MovementTests(unittest.TestCase):
 def test_real_shadow_samples_produce_transitions_without_eligibility(self):
  ...
  value=report(paths);self.assertGreater(value['transition_count'],0);self.assertGreater(value['top_quote_changed_transition_count'],0);self.assertNotIn('pricing_eligible',json.dumps(value))
