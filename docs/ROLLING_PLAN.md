...
> **Canonical source:** [`docs/rolling-plan.json`](rolling-plan.json) · **Last verified:** 2026-09-01
...
- **Next action:** `P5-07: continue scheduled immutable collection beyond the 466.67-hour scan-artifact window and 465.87-hour paired-book window, independently review the four source-identifier date conflicts and the broader corpus, add host/process uptime evidence, and do not advance go/no-go until representative evidence exists.`
...
- [ ] **P3-08 — Meet matching release gate** `[Blocked]`
  ...
  - **Evidence:** `Requires a sufficiently sized independently reviewed labeled corpus; executable cases and the replay corpus verify deterministic decisions, but no independent reviewer has labeled a sufficiently sized corpus for the 99% precision gate. data/reports/source-identifier-audit.json adds an implementation-independent automated cross-check of 273 retained REVIEW pairs: identifiers corroborate 269 and retain four date conflicts requiring independent review. The report explicitly cannot satisfy the independent-label gate and enables no pricing.`
...
- [x] **P4-05 — Measure and set freshness thresholds** `[Done]`
  ...
  - **Evidence:** `docs/freshness-policy.md and config/freshness/mlb-books-2026-08-12-v1.json derive a conservative 800ms pair-skew limit from the original 220-observation checkpoint; the 2026-09-01 elapsed checkpoint has 217,634 successful samples across 538 pairs over 465.87h, p99 602ms and a retained 30.79s fail-closed outlier. Polymarket source-age p99 is 3,477.08s and remains measurement-only, so the existing reviewed limits are not loosened or expanded pending a new policy version.`
...
- [x] **P5-06 — Audit post-resolution agreement** `[Done]`
  ...
  - **Evidence:** `src/arbs/resolution_audit.py and scripts/audit_resolutions.py retain deduplicated historical matches after catalog rollover, cross-check source identifiers before fetching outcomes, hash public venue evidence, recognize Kalshi finalized status, select one Polymarket moneyline, require unambiguous winners, and fail closed. Live 2026-09-01 audit covered 273 historical matches from 4,070 validated reports: 222 are comparable finals with 222 agreements and 0 divergences, while 51 pending, unknown or identifier-conflicted matches remain fail-closed; data/reports/resolution-audit.json is READY_FOR_REVIEW and remains pricing-ineligible.`
- [ ] **P5-07 — Complete sustained shadow run** `[In progress]`
  ...
  - **Evidence:** `data/reports/shadow-validation-checkpoint.json records 217,634 successful paired-book observations across 538 pairs over 1,677,118 seconds (465.87h), with 217,096 subsequent transitions, 37,858 top-quote changes and 13 fail-closed samples. data/reports/resolution-audit.json covers 273 retained historical matches, with 222/222 comparable final-outcome agreements, 0 divergences and 51 pending, unknown or identifier-conflicted matches kept fail-closed. Operational tooling measures 4,071 valid scan artifacts over 1,680,009 seconds (466.67h), 3,719/5,601 occupied five-minute slots (66.40%) and a 9,211.95s largest observed gap, explicitly scoped as artifact coverage rather than host/process uptime; it confirms all 273 unique event pairs remain REVIEW, zero are pricing-eligible, and modeled net results are not computable. An independent parser implementation cross-checks source identifiers for all 273 pairs, corroborating 269 while quarantining four date conflicts; the resolution audit excludes those conflicts, and this automated evidence is explicitly not independent review. This is durable partial latency, movement, resolution, parser-decision and collection-continuity evidence, but not yet a representative window or several hundred independently reviewed events; independent false-positive review, host/process uptime and eligible modeled net results remain unavailable.`
- [ ] **P5-08 — Pass go/no-go review for execution design** `[Blocked]`
  ...
  - **Evidence:** `Blocked: P3-08 lacks a sufficiently sized independently reviewed corpus and P5-07 still lacks a representative window, several hundred independently reviewed events, host/process uptime, false-positive review and eligible modeled net results. Artifact coverage is only 3,719/5,601 observed five-minute slots with a 9,211.95s maximum gap; all 273 unique event pairs remain REVIEW/pricing-disabled, and four source-identifier date conflicts require independent review. P5-06 has 222/222 comparable final agreements with 51 retained matches pending, unknown or identity-conflicted, but no signed go/no-go summary exists.`
